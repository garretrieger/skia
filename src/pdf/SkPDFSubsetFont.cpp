...
static sk_sp<SkData> subset_harfbuzz(sk_sp<SkData> fontData,
    ...
    hb_subset_input_set_flag(input.get(), HB_SUBSET_FLAG_RETAIN_GIDS, true);
    // TODO: When possible, check if a font is 'tricky' with FT_IS_TRICKY.
    // If it isn't known if a font is 'tricky', retain the hints.
    hb_subset_input_set_flag(input.get(), HB_SUBSET_FLAG_HINTING, true);
    hb_face_t* subset_face = hb_subset_or_fail(face.get(), input.get());
#else
    hb_subset_input_set_retain_gids(input.get(), true);
    // TODO: When possible, check if a font is 'tricky' with FT_IS_TRICKY.
    // If it isn't known if a font is 'tricky', retain the hints.
    hb_subset_input_set_drop_hints(input.get(), false);
    hb_face_t* subset_face = hb_subset(face.get(), input.get());
#endif
    if (!subset_face) {
      return nullptr;
    }
    HBFace subset(subset_face);
